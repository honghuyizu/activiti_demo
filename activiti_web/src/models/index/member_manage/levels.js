...
import { getPageList, getBean, add, edit, remove, removeBatch, changeOrderBy } from '../../../services/index/member_manage/levels';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        } else if (location.pathname === '/index/member_manage/levels/add') {
          dispatch({
            type: 'initAdd'
          });
        } else if (pathToRegexp('/index/member_manage/levels/level/:id').test(location.pathname)) {
  ...
  effects: {
    ...
    *initAdd({ payload }, { call, put }) {
      yield put({ type: 'setValue', payload: { curAction: 'add', curActionName: '新增级别', curRecord: {}, actionBarVisible: false } });
    },
    *initEdit({ payload }, { call, put }) {
      ...
      if (data) {
        console.log('data:', JSON.stringify(data));
        if (data.code === projectConstant.CODE_SUCCESS) {
          yield put({ type: 'setValue', payload: { curAction: 'edit', curActionName: '编辑级别', curRecord: data.data, actionBarVisible: false } });
        }
      }
    },

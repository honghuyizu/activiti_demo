...
import { getPageList, getBean, add, edit, remove, removeBatch } from '../../../../services/index/content_manage/ad_manage/adPositions';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        } else if (location.pathname === '/index/content_manage/ad_manage/ad_positions/add') {
          dispatch({
            type: 'initAdd'
          });
        } else if (pathToRegexp('/index/content_manage/ad_manage/ad_positions/ad_position/:id').test(location.pathname)) {
  ...
  effects: {
    ...
    *initAdd({ payload }, { call, put }) {
      yield put({ type: 'setValue', payload: { curAction: 'add', curActionName: '新增广告位', curRecord: {}, actionBarVisible: false } });
    },
    *initEdit({ payload }, { call, put }) {
      ...
      if (data) {
        console.log('data:', JSON.stringify(data));
        if (data.code === projectConstant.CODE_SUCCESS) {
          yield put({ type: 'setValue', payload: { curAction: 'edit', curActionName: '编辑广告位', curRecord: data.data, actionBarVisible: false } });
        }
      }
    },

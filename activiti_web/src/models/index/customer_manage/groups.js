...
import { getPageList, getBean, add, edit, remove, removeBatch, changeOrderBy } from '../../../services/index/customer_manage/groups';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        } else if (location.pathname === '/index/customer_manage/groups/add') {
          dispatch({
            type: 'initAdd'
          });
        } else if (pathToRegexp('/index/customer_manage/groups/group/:id').test(location.pathname)) {
  ...
  effects: {
    ...
    *initAdd({ payload }, { call, put }) {
      yield put({ type: 'setValue', payload: { curAction: 'add', curActionName: '新增分组', curRecord: {}, actionBarVisible: false } });
    },
    *initEdit({ payload }, { call, put }) {
      ...
      if (data) {
        console.log('data:', JSON.stringify(data));
        if (data.code === projectConstant.CODE_SUCCESS) {
          yield put({ type: 'setValue', payload: { curAction: 'edit', curActionName: '编辑分组', curRecord: data.data, actionBarVisible: false } });
        }
      }
    },

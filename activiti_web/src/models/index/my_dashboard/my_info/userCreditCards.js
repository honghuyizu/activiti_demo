...
import { getPageList, getBean, add, edit, remove, removeBatch } from '../../../../services/index/my_dashboard/my_info/userCreditCards';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        } else if (location.pathname === "/index/my_dashboard/my_info/user_credit_cards/add") {
          dispatch({
            type: 'initAdd'
          });
        } else if (pathToRegexp("/index/my_dashboard/my_info/user_credit_cards/user_credit_card/:id").test(location.pathname)) {
  ...
  effects: {
    ...
    *initAdd({ payload }, { call, put }) {
      yield put({ type: 'setValue', payload: { curAction: 'add', curActionName: "新增我的信用卡", curRecord: {}, actionBarVisible: false } });
    },
    *initEdit({ payload }, { call, put }) {
      ...
      if (data) {
        console.log('data:', JSON.stringify(data));
        if (data.code === projectConstant.CODE_SUCCESS) {
          yield put({ type: 'setValue', payload: { curAction: 'edit', curActionName: "编辑我的信用卡", curRecord: data.data, actionBarVisible: false } });
        }
      }
    },

...
import { getPageList, getBean, add, edit, remove, removeBatch, changeOrderBy } from '../../../../services/index/system_config/mobile_app_manage/mobileApps';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        } else if (location.pathname === '/index/system_config/mobile_app_manage/mobile_apps/add') {
          dispatch({
            type: 'initAdd'
          });
        } else if (pathToRegexp('/index/system_config/mobile_app_manage/mobile_apps/mobile_app/:id').test(location.pathname)) {
  ...
  effects: {
    ...
    *initAdd({ payload }, { call, put }) {
      yield put({ type: 'setValue', payload: { curAction: 'add', curActionName: '新增应用', curRecord: {}, actionBarVisible: false } });
    },
    *initEdit({ payload }, { call, put }) {
      ...
      if (data) {
        console.log('data:', JSON.stringify(data));
        if (data.code === projectConstant.CODE_SUCCESS) {
          yield put({ type: 'setValue', payload: { curAction: 'edit', curActionName: '编辑应用', curRecord: data.data, actionBarVisible: false } });
        }
      }
    },

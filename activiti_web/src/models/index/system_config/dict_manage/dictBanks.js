...
import { getPageList, getBean, add, edit, remove, removeBatch } from '../../../../services/index/system_config/dict_manage/dictBanks';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        } else if (location.pathname === "/index/system_config/dict_manage/dict_banks/add") {
          dispatch({
            type: 'initAdd'
          });
        } else if (pathToRegexp("/index/system_config/dict_manage/dict_banks/dict_bank/:id").test(location.pathname)) {
  ...
  effects: {
    ...
    *initAdd({ payload }, { call, put }) {
      yield put({ type: 'setValue', payload: { curAction: 'add', curActionName: "新增银行", curRecord: {}, actionBarVisible: false } });
    },

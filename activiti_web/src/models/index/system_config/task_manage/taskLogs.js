...
import { getPageList, getBean, add, edit, remove, removeBatch } from '../../../../services/index/system_config/task_manage/taskLogs';
import { getList as getTaskList } from '../../../../services/index/system_config/task_manage/tasks';
...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        } else if (location.pathname === '/index/system_config/task_manage/task_logs/add') {
          dispatch({
            type: 'initAdd'
          });
        } else if (pathToRegexp('/index/system_config/task_manage/task_logs/task_log/:id').test(location.pathname)) {
  ...
  effects: {
    *initList({ payload }, { call, put }) {
      ...
      // 得到定时任务列表
      let curTaskID = (parse(payload).task_id);
    ...
    *initAdd({ payload }, { call, put }) {
      yield put({ type: 'setValue', payload: { curAction: 'add', curActionName: '新增任务日志', curRecord: {}, actionBarVisible: false } });
    },
    *initEdit({ payload }, { call, put }) {
      ...
      if (data) {
        console.log('data:', JSON.stringify(data));
        if (data.code === projectConstant.CODE_SUCCESS) {
          yield put({ type: 'setValue', payload: { curAction: 'edit', curActionName: '编辑任务日志', curRecord: data.data, actionBarVisible: false } });
        }
      }
    },
